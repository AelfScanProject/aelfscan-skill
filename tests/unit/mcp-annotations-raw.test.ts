...
describe('MCP raw annotation contract', () => {
  ...
  test('raw tools/list includes both camelCase and snake_case read-only hints', async () => {
    ...
    const searchTool = tools.find(tool => tool.name === 'aelfscan_search');
    const blocksTool = tools.find(tool => tool.name === 'aelfscan_blocks');

    expect(searchTool?.annotations?.readOnlyHint).toBe(true);
    expect(searchTool?.annotations?.read_only_hint).toBe(true);
    expect(searchTool?.annotations?.destructiveHint).toBeUndefined();

    expect(blocksTool?.annotations?.readOnlyHint).toBe(true);
    expect(blocksTool?.annotations?.read_only_hint).toBe(true);
    expect(blocksTool?.annotations?.destructiveHint).toBeUndefined();
  });
});
