...
describe('IronClaw setup workflow', () => {
  ...
  beforeEach(() => {
    testRoot = join(tmpdir(), `aelfscan-ironclaw-${Date.now()}-${Math.random()}`);
  ...
  test('setup writes trusted skill and MCP config without creating installed_skills', () => {
    ...
    expect(readFileSync(installedSkillPath, 'utf8')).toContain('aelfscan-skill');
  });
  ...
  test('re-running setup without force keeps a single MCP entry and skill file', () => {
    ...
    expect(config.servers.filter(server => server.name === 'aelfscan-skill')).toHaveLength(1);
  });
  ...
  test('defensive removal rejects unexpected skill paths', () => {
    const unexpectedSkillDir = join(testRoot, 'unexpected', 'aelfscan-skill');
